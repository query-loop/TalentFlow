...
{:else}
  <div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold flex items-center gap-2"><Icon name="tag"/> Keywords — <span class="text-sm text-gray-600">{pipeline.name}</span></h1>
    ...
    <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-5 bg-white dark:bg-gray-900 space-y-2">
      <div class="text-sm text-gray-600 dark:text-gray-300">This is a placeholder for the pipeline-scoped Keywords page. Use the global Keywords page for now.</div>
      <a class="text-sm text-blue-600 hover:underline" href="/app/keywords">Go to global Keywords</a>
    </div>

    <StepFooter current="keywords" pipelineId={pipeline.id} />
  </div>
{/if}
