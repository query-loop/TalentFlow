...
{:else if pipe}
  <div class="p-4 md:p-6 space-y-4">
    ...
    <div class="border rounded-lg bg-white dark:bg-slate-800 p-4">
      {#if pipe.artifacts && (pipe.artifacts as any).scoring?.strengths?.length}
        <ul class="space-y-2">
          {#each (pipe.artifacts as any).scoring.strengths as strength}
      ...
      {:else}
        <div class="text-sm text-slate-500 dark:text-slate-400">No differentiators identified yet. Run scoring on the JD step.</div>
      {/if}
    </div>
  </div>
{/if}
